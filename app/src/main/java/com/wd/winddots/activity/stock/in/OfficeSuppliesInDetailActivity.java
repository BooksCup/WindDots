...
public class OfficeSuppliesInDetailActivity extends BaseActivity implements StockGoodsSpecAdapter.TextChangeListener {
    ...
    @OnClick({R.id.iv_back, R.id.ll_goods_content,
            R.id.ll_ware_house, R.id.tv_draft, R.id.tv_scan,
            R.id.tv_submit})
    ...
    private void initView() {
        ...
        mRequestRole = getIntent().getStringExtra("requestRole");
        if (Constant.REQUEST_ROLE_CONFIRM.equals(mRequestRole)) {
            // 入库操作
            mDraftTv.setText("保存");
            mConfirmTv.setVisibility(View.VISIBLE);
        } else {
            // 入库申请
            mDraftTv.setText("保存至草稿");
