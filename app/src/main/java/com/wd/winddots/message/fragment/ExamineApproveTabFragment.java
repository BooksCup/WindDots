...
import com.wd.winddots.message.presenter.impl.ExamineApprovePresenterImpl;
import com.wd.winddots.message.presenter.view.ExamineApproveView;
import com.wd.winddots.mvp.widget.ApprovalProcessActivity;
import com.wd.winddots.message.adapter.ExamineApproveAdapter;
...
public class ExamineApproveTabFragment extends BaseFragment<ExamineApproveView, ExamineApprovePresenterImpl> implements ExamineApproveView, SwipeRefreshLayout.OnRefreshListener, BaseQuickAdapter.RequestLoadMoreListener, BaseQuickAdapter.OnItemClickListener {
    ...
    private ExamineApproveTabUnread examineApproveTabUnread;

    public void setExamineApproveTabUnread(ExamineApproveTabUnread examineApproveTabUnread) {
        this.examineApproveTabUnread = examineApproveTabUnread;
    }
    ...
    public void onExamineApproveListRefreshSuccess(ExamineApproveBean bean) {
        ...
        for (int i = 0;i < list.size();i++){
            ExamineApproveBean.ListBean bean1 = list.get(i);
            if (2 == bean1.getReadStatus()){
                unread += 1;
            }
        }
        if (examineApproveTabUnread != null){
            examineApproveTabUnread.setExamineApproveTabUnread(unread);
        }
    }
    ...
    public void onExamineApproveListLoadMoreSuccess(ExamineApproveBean bean) {
        ...
        for (int i = 0;i < mListBeanList.size();i++){
            ExamineApproveBean.ListBean bean1 = mListBeanList.get(i);
            if (2 == bean1.getReadStatus()){
                unread += 1;
            }
        }
    ...
    public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
        ...
        presenter.changeApplyReadStatus(id);
    ...
    public interface ExamineApproveTabUnread{
        void setExamineApproveTabUnread(int unread);
    }
}
